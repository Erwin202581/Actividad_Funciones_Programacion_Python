        ...
        def wrapper(*args, **kwargs): 
            ...
            for i in range(1, max_retries + 1): 
                try: 
                    ...
                    if i < max_retries: 
                        # Añadir 'jitter' (aleatoriedad) para evitar la sincronización de clientes
...
class ServicioSimulado: 
    ...
    def realizar_llamada_api(self, endpoint): 
        ...
        # Lógica de fallo: falla las primeras 3 llamadas, luego tiene una probabilidad
        if self.call_count < 4 or random.random() < self.fail_probability:
             if self.call_count < 5: # Falla hasta el cuarto intento para asegurar el éxito en el 5to
