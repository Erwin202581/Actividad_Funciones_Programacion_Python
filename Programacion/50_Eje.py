...
# Nota: En versiones recientes, 'hmset' está obsoleto, pero se mantiene por compatibilidad. 
# Si usas Python 3.x y redis-py, la sintaxis con diccionario funciona bien.
# Sin embargo, 'hmset' espera strings/bytes, no diccionarios directamente. 
# Usaremos hset por ser el método recomendado para nuevas implementaciones.

# r.hmset(session_id, user_data) # Original, pero deprecated

r.hset(session_id, mapping=user_data) # Método moderno para establecer múltiples campos
...
print(f"Campo 'last_login' actualizado en la sesión '{session_id}'.") 
